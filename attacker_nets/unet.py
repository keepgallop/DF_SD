...
def autocrop(encoder_layer: torch.Tensor, decoder_layer: torch.Tensor):
    ...
    if encoder_layer.shape[2:] != decoder_layer.shape[2:]:
        ...
        if encoder_layer.dim() == 4:  # 2D
            encoder_layer = encoder_layer[:, :, ((ds[0] - es[0]) //
                                                 2):((ds[0] + es[0]) // 2),
                                          ((ds[1] - es[1]) //
                                           2):((ds[1] + es[1]) // 2), ]
...
def get_up_layer(
    ...
    if up_mode == 'transposed':
        return conv_transpose_layer(dim)(in_channels,
                                         out_channels,
                                         kernel_size=kernel_size,
                                         stride=stride)
    else:
...
def get_maxpool_layer(kernel_size: int = 2,
                      ...
                      padding: int = 0,
                      dim: int = 2):
    return maxpool_layer(dim=dim)(kernel_size=kernel_size,
                                  stride=stride,
                                  padding=padding)
...
def get_normalization(normalization: str, num_channels: int, dim: int):
    if normalization == 'batch':
        ...
        num_groups = int(normalization.partition('group')
                         [-1])  # get the group size from string
...
class DownBlock(nn.Module):
    ...
    def __init__(
        ...
        # pooling layer
        if self.pooling:
            self.pool = get_maxpool_layer(kernel_size=2,
                                          stride=2,
                                          padding=0,
                                          dim=self.dim)
...
class UpBlock(nn.Module):
    ...
    def forward(self, encoder_layer, decoder_layer):
        ...
        cropped_encoder_layer, dec_layer = autocrop(encoder_layer,
                                                    up_layer)  # cropping
        ...
        merged_layer = self.concat(up_layer,
                                   cropped_encoder_layer)  # concatenation
    ...
    def initialize_parameters(
        ...
        for module in self.modules():
            self.weight_init(module, method_weights,
                             **kwargs_weights)  # initialize weights
            self.bias_init(module, method_bias,
                           **kwargs_bias)  # initialize bias
