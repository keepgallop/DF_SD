...
parser.add_argument(
    ...
    default='',
    nargs='+',
    help='space separated metrics list: psnr | ssim | lpips | fid | lfd | mse')
parser.add_argument('--logs',
                    type=str,
                    default=None,
                    help='path to log file to record scores')

# 1. single sample test
parser.add_argument('--imgf',
                    type=str,
                    default=None,
...
parser.add_argument('--fdf',
                    type=str,
                    default=None,
                    help='path to folder of fake images')
...
parser.add_argument('--fdrf',
                    type=str,
                    default=None,
                    help='path to folder of real and fake image pairs')

opt = parser.parse_args()

for i in range(len(opt.metrics)):
    opt.metrics[i] = opt.metrics[i].lower()
    assert opt.metrics[i] in [
        'psnr', 'ssim', 'lpips', 'fid', 'lfd', 'mse'
...
if opt.fdf is not None and opt.fdr is not None:
    ...
    for metric in opt.metrics:
        ...
        print_and_write_log('%s: %.10f' % (metric.upper(), result_mean),
                            opt.logs)
...
if opt.fdrf is not None:
    ...
    for i in tqdm(range(len(listrf))):
        ...
        for metric in opt.metrics:
            if metric == 'fid':
                cv2.imwrite(f'{tmp_path}/fake/{i:08d}.png',
                            img_fake.astype(np.uint8))
                cv2.imwrite(f'{tmp_path}/real/{i:08d}.png',
                            img_real.astype(np.uint8))
            else:
                result = eval(metric + '(img_fake, img_real)')
                results[metric].append(result)
    if 'fid' in opt.metrics:
